...
<div class="min-h-screen flex items-center justify-center pt-24 px-4">
    <div class="w-full max-w-lg bg-white rounded-xl shadow-2xl p-8">
        ...
        <form method="POST" action="{{ route('comercios.update', $comercio->id_comercio) }}" enctype="multipart/form-data" class="space-y-4">
            @csrf
            @method('PUT') 
            ...
            <div>
                <label class="block mb-2 text-sm font-medium text-gray-900">Actualizar Foto (Opcional)</label>
                @if($comercio->foto)
                    <img src="{{ asset('storage/' . $comercio->foto) }}" class="h-20 w-20 object-cover rounded-lg mb-2 shadow">
                @endif
                <input type="file" name="foto" accept="image/*" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg block w-full p-2">
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block mb-2 text-sm font-medium text-gray-900">Ciudad</label>
                    <input type="text" name="ciudad" value="{{ $comercio->ciudad }}"
                        class="bg-gray-50 border border-gray-300 rounded-lg w-full p-2.5">
                </div>
                <div>
                    <label class="block mb-2 text-sm font-medium text-gray-900">Dirección</label>
                    <input type="text" name="direccion" value="{{ $comercio->direccion }}"
                        class="bg-gray-50 border border-gray-300 rounded-lg w-full p-2.5">
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="time" name="horario_apertura" value="{{ $comercio->horario_apertura }}"
                        class="bg-gray-50 border border-gray-300 rounded-lg w-full p-2.5">
                </div>
                <div>
                    <label class="block mb-2 text-sm font-medium text-gray-900">Cierre</label>
                    <input type="time" name="horario_cierre" value="{{ $comercio->horario_cierre }}"
                        class="bg-gray-50 border border-gray-300 rounded-lg w-full p-2.5">
                </div>
            </div>
            ...
            <div class="flex items-center">
                <input id="activo" name="activo" type="checkbox" value="1"
                    {{ $comercio->activo ? 'checked' : '' }} class="w-4 h-4 border-gray-300 rounded text-blue-600 focus:ring-blue-500">
                <label for="activo" class="ml-2 text-sm font-medium text-gray-900">Activo</label>
            </div>

            <div class="flex gap-4 pt-4">
                <a href="{{ route('comercios.index') }}"
                    class="w-1/2 text-center text-gray-700 bg-gray-200 hover:bg-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 transition">Cancelar</a>
                <button type="submit"
                    class="w-1/2 text-white bg-blue-700 hover:bg-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 transition">Actualizar</button>
            </div>
        </form>
    </div>
</div>
